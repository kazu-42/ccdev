    ...
    return (
      <div className="p-4 border-b border-gray-700">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-gray-500">@{githubStatus.username}</span>
        </div>
        <button
    ...
    <div className="p-4 border-b border-gray-700">
      ...
      {gitStatus && (
        <div className="mb-3 space-y-1 text-xs">
          {gitStatus.ahead > 0 && (
            <div className="text-blue-400">{gitStatus.ahead} commit(s) ahead</div>
          )}
          {gitStatus.behind > 0 && (
            <div className="text-yellow-400">{gitStatus.behind} commit(s) behind</div>
          )}
          {gitStatus.staged.length > 0 && (
            <div className="text-green-400">{gitStatus.staged.length} staged</div>
          )}
          {gitStatus.modified.length > 0 && (
            <div className="text-orange-400">{gitStatus.modified.length} modified</div>
          )}
          {gitStatus.untracked.length > 0 && (
            <div className="text-gray-400">{gitStatus.untracked.length} untracked</div>
          )}
