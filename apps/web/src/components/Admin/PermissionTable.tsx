...
export function PermissionTable() {
  const { permissions, permissionsLoading, users, deletePermission, createPermission } = useAdminStore();
    ...
    <div>
      ...
      {showAddForm && (
        <form onSubmit={handleCreate} className="mb-6 p-4 bg-dark-bg rounded-lg border border-dark-border">
          <h3 className="text-sm font-medium text-white mb-4">Add New Permission</h3>
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setNewPermission({ ...newPermission, resource_type: e.target.value as Permission['resource_type'] })}
                className="w-full bg-dark-surface border border-dark-border rounded px-3 py-2 text-sm text-white"
              >
            ...
            <div>
              <label className="block text-xs text-gray-400 mb-1">Resource ID (optional)</label>
              <input
                type="text"
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setNewPermission({ ...newPermission, action: e.target.value as Permission['action'] })}
                className="w-full bg-dark-surface border border-dark-border rounded px-3 py-2 text-sm text-white"
              >
          ...
          <table className="w-full">
            <thead>
              <tr className="text-left border-b border-dark-border">
                ...
                <th className="px-4 py-3 text-xs font-medium text-gray-400 uppercase tracking-wider">Resource Type</th>
                <th className="px-4 py-3 text-xs font-medium text-gray-400 uppercase tracking-wider">Resource ID</th>
            ...
            <tbody className="divide-y divide-dark-border">
              {permissions.map((permission) => (
                <tr key={permission.id} className="hover:bg-dark-hover transition-colors">
                  <td className="px-4 py-3">
                    <code className="text-xs text-gray-400">{permission.user_id.slice(0, 8)}...</code>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-400">
                    {permission.resource_id || <span className="text-gray-600">All</span>}
                  </td>
                  <td className="px-4 py-3">
